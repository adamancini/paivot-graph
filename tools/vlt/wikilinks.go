...
// wikilink represents a parsed [[...]] reference in a note.
...
// wikiLinkPattern matches all wikilinks: [[Title]], [[Title#Heading]],
// [[Title|Display]], [[Title#Heading|Display]].
var wikiLinkPattern = regexp.MustCompile(`\[\[([^\]#|]+?)(?:#([^\]|]*))?(?:\|([^\]]*))?\]\]`)

// parseWikilinks extracts all wikilinks from text.
...
// replaceWikilinks replaces all wikilinks referencing oldTitle with newTitle,
// preserving any #heading and |display text. Case-insensitive to match
// Obsidian's link resolution behavior.
func replaceWikilinks(text, oldTitle, newTitle string) string {
	pattern := regexp.MustCompile(
		`(?i)\[\[` + regexp.QuoteMeta(oldTitle) +
			...
			`((?:\|[^\]]*)?)` +
			`\]\]`)
	return pattern.ReplaceAllString(text, `[[`+newTitle+`${1}${2}]]`)
}
...
// findBacklinks returns relative paths of notes that contain wikilinks to
// the given title. Case-insensitive to match Obsidian's resolution.
